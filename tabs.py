...
# ==================== NUOVE FUNZIONI PER ACK ====================

def format_delivery_time(seconds):
    # Formatta il tempo di consegna in modo leggibile
    if seconds is None:
        return "-"
    if seconds < 1:
...
def calculate_success_rate(history):
    ...
    if not history:
        return 0
    delivered = sum(1 for m in history if m.get('status') == 'delivered')
    return (delivered / len(history)) * 100